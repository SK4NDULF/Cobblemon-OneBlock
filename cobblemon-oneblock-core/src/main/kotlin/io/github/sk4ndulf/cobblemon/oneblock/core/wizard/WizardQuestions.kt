...
class WizardQuestion(
    ...
    val questionLangKey: String get() = "cobblemon_oneblock.question.$key"
    val labelLangKey: String get() = "cobblemon_oneblock.label.$key"
}
...
object WizardQuestions {
    ...
    fun boolText(value: Boolean): String =
        ServerLang.raw(if (value) "cobblemon_oneblock.value.yes" else "cobblemon_oneblock.value.no")
    ...
    private fun intQuestion(
        ...
        when {
            value == null -> AnswerResult.Error(ServerLang.raw("cobblemon_oneblock.wizard.invalid_number", raw.trim()))
            value < min || value > max -> AnswerResult.Error(ServerLang.raw("cobblemon_oneblock.wizard.out_of_range", min, max))
    ...
    private fun boolQuestion(
        ...
        when (val value = parseBool(raw)) {
            null -> AnswerResult.Error(ServerLang.raw("cobblemon_oneblock.wizard.invalid_bool"))
            else -> AnswerResult.Ok(set(config, value))
        }
    })
