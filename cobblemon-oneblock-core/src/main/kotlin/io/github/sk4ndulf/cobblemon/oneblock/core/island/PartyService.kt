...
object PartyService {
    ...
    fun invite(owner: ServerPlayer, target: ServerPlayer) {
        ...
        if (island == null || island.owner() != owner.uuid) {
            owner.sendSystemMessage(ServerLang.msg(if (island == null) "cobblemon_oneblock.island.none" else "cobblemon_oneblock.island.only_owner"))
            return
        }
        if (target.uuid == owner.uuid) {
            owner.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.invite_self"))
            return
        }
        if (manager.islandDataOf(target.uuid) != null) {
            owner.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.target_has_island", target.gameProfile.name))
            return
        }
        val maxSize = OneBlockCore.configManager.mainConfig.maxPartySize
        if (island.memberSet.size + 1 >= maxSize) {
            owner.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.full", maxSize))
            return
        }
        ...
        owner.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.invited_sender", target.gameProfile.name, timeoutSeconds))
        target.sendSystemMessage(
            ServerLang.msg("cobblemon_oneblock.party.invited_target", owner.gameProfile.name)
                .withStyle(ChatFormatting.GOLD)
                .append(" ")
                .append(button("cobblemon_oneblock.party.button.accept", "/ob party accept", ChatFormatting.GREEN))
                .append(" ")
                .append(button("cobblemon_oneblock.party.button.decline", "/ob party deny", ChatFormatting.RED)),
        )
    }
    ...
    fun accept(player: ServerPlayer) {
        ...
        if (invite == null || invite.expiresAt < System.currentTimeMillis()) {
            player.sendSystemMessage(ServerLang.msg(if (invite == null) "cobblemon_oneblock.party.no_invite" else "cobblemon_oneblock.party.invite_expired"))
            return
        }
        val island = manager.islandDataById(invite.islandId)
        if (island == null || island.state != IslandState.ACTIVE) {
            player.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.invite_expired"))
            return
        }
        if (manager.islandDataOf(player.uuid) != null) {
            player.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.island.already_have"))
            return
        }
        val maxSize = OneBlockCore.configManager.mainConfig.maxPartySize
        if (island.memberSet.size + 1 >= maxSize) {
            player.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.full", maxSize))
            return
        }
        manager.addMember(island, player.uuid)
        player.sendSystemMessage(
            ServerLang.msg("cobblemon_oneblock.party.joined_self", invite.inviterName).withStyle(ChatFormatting.GREEN),
        )
        broadcast(island, player.server, "cobblemon_oneblock.party.joined_broadcast", player.gameProfile.name, exclude = player.uuid)
    }

    fun deny(player: ServerPlayer) {
        val invite = invites.remove(player.uuid)
        if (invite == null) {
            player.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.no_invite"))
            return
        }
        player.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.denied"))
        player.server.playerList.players
            .firstOrNull { it.gameProfile.name == invite.inviterName }
            ?.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.denied_owner", player.gameProfile.name))
    }
    ...
    fun kick(owner: ServerPlayer, target: GameProfile) {
        ...
        if (!manager.removeMember(island, target.id, PartyLeaveEvent.Reason.KICKED)) {
            owner.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.not_member", target.name))
            return
        }
        owner.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.kicked_owner", target.name))
        owner.server.playerList.getPlayer(target.id)?.let { online ->
            HubManager.sendToHub(online)
            online.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.kicked_target", owner.gameProfile.name))
        }
    }
    ...
    fun leave(player: ServerPlayer) {
        ...
        if (island == null) {
            player.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.island.none"))
            return
        }
        if (island.owner() == player.uuid) {
            player.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.owner_cant_leave"))
            return
        }
        manager.removeMember(island, player.uuid, PartyLeaveEvent.Reason.LEFT)
        HubManager.sendToHub(player)
        player.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.left_self"))
        broadcast(island, player.server, "cobblemon_oneblock.party.left_broadcast", player.gameProfile.name, exclude = player.uuid)
    }
    ...
    fun list(player: ServerPlayer) {
        ...
        player.sendSystemMessage(
            ServerLang.msg("cobblemon_oneblock.party.list_header", ownerName, island.memberSet.size + 1, maxSize)
                .withStyle(ChatFormatting.GOLD),
        )
        player.sendSystemMessage(ServerLang.msg("cobblemon_oneblock.party.list_entry", ownerName, ServerLang.raw("cobblemon_oneblock.role.owner")))
        for (member in island.memberSet) {
            player.sendSystemMessage(
                ServerLang.msg("cobblemon_oneblock.party.list_entry", nameOf(server, member), ServerLang.raw("cobblemon_oneblock.role.member")),
            )
        }
    }
