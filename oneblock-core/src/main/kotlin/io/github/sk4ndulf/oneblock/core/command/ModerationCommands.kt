...
object ModerationCommands {
    ...
    private fun ban(context: CommandContext<CommandSourceStack>, reason: String?): Int {
        ...
        val effectiveReason = reason ?: ServerLang.raw("oneblock.ban.no_reason")
        if (!BanService.ban(island, profile.id, actor?.uuid, effectiveReason)) {
            context.source.sendFailure(ServerLang.msg("oneblock.ban.cannot_ban_member", profile.name))
            return 0
        }
        context.source.sendSuccess(
            { ServerLang.msg("oneblock.ban.banned", profile.name).withStyle(ChatFormatting.GREEN) }, false,
        )
        context.source.server.playerList.getPlayer(profile.id)?.let { online ->
            ...
            online.sendSystemMessage(ServerLang.msg("oneblock.ban.you_were_banned", effectiveReason))
        }
    ...
    private fun unban(context: CommandContext<CommandSourceStack>): Int {
        ...
        if (!BanService.unban(island.id, profile.id)) {
            context.source.sendFailure(ServerLang.msg("oneblock.ban.not_banned", profile.name))
            return 0
        }
        context.source.sendSuccess({ ServerLang.msg("oneblock.ban.unbanned", profile.name) }, false)
    ...
    private fun listBans(context: CommandContext<CommandSourceStack>): Int {
        ...
        context.source.sendSystemMessage(
            ServerLang.msg("oneblock.ban.list_header", entries.size).withStyle(ChatFormatting.GOLD),
        )
        for ((uuid, ban) in entries) {
            context.source.sendSystemMessage(
                ServerLang.msg("oneblock.ban.list_entry", nameOf(context, uuid), ban.reason),
            )
        }
        return Command.SINGLE_SUCCESS
    }
    ...
    private fun adminKick(context: CommandContext<CommandSourceStack>): Int {
        ...
        if (online == null) {
            context.source.sendFailure(ServerLang.msg("oneblock.admin.not_online", profile.name))
            return 0
        }
        HubManager.sendToHub(online)
        online.sendSystemMessage(ServerLang.msg("oneblock.admin.kicked_to_hub"))
        context.source.sendSuccess({ ServerLang.msg("oneblock.admin.kick_done", profile.name) }, true)
    ...
    private fun adminInfo(context: CommandContext<CommandSourceStack>): Int {
        ...
        context.source.sendSystemMessage(
            ServerLang.msg("oneblock.admin.info_header", island.id, island.slot()).withStyle(ChatFormatting.GOLD),
        )
        context.source.sendSystemMessage(
            ServerLang.msg("oneblock.admin.info_location", anchor.x, anchor.y, anchor.z),
        )
        context.source.sendSystemMessage(
            ServerLang.msg("oneblock.admin.info_progress", island.borderLevel, size, island.breakCount),
        )
        context.source.sendSystemMessage(
            ServerLang.msg("oneblock.admin.info_party", island.memberSet.size + 1, config.maxPartySize),
        )
        return Command.SINGLE_SUCCESS
    }
    ...
    private fun requestForce(context: CommandContext<CommandSourceStack>, action: String): Int {
        ...
        context.source.sendSystemMessage(
            ServerLang.msg("oneblock.admin.confirm_prompt", verb, profile.name, island.id)
    ...
    private fun confirmForce(context: CommandContext<CommandSourceStack>, reset: Boolean): Int {
        ...
        if (reset) {
            ...
            if (online != null) {
                ...
                context.source.sendSystemMessage(ServerLang.msg("oneblock.admin.reset_offline", profile.name))
            }
        } else {
            server.playerList.getPlayer(ownerId)?.let { HubManager.sendToHub(it) }
        }
        context.source.sendSuccess(
            { ServerLang.msg("oneblock.admin.force_done", if (reset) "reset" else "delete", profile.name) }, true,
        )
